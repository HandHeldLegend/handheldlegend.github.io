...
consoleStartButton.addEventListener('click', async () => {
  if (device === null) {
    device = await navigator.serial.requestPort({});
    transport = new Transport(device, true);
  }
  ...
  await transport.connect(globalBaudInt);
...
consoleStopButton.addEventListener('click', async () => {
...
programButton.addEventListener('click', async () => {
  ...
  for (let index = 1; index < table.rows.length; index++) {
    ...
    const fileObj = row.cells[1].childNodes[0] & { data: string };
