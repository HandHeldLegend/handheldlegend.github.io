...
class SwitchController {
    ...
    parseFactoryCalibrationData(leftRight, spiData)
    {
        if (leftRight === 'left') {
            ...
            // Center and max are mixed up here.
            this.factoryCalibration.rightStick.x.max = ((spiData[1] << 8) & 0xF00) | spiData[0];
            this.factoryCalibration.rightStick.y.max = (spiData[2] << 4) | (spiData[1] >> 4);

            this.factoryCalibration.rightStick.x.center = ((spiData[4] << 8) & 0xF00) | spiData[3];
            this.factoryCalibration.rightStick.y.center = (spiData[5] << 4) | (spiData[4] >> 4);
    ...
    parseUserCalibrationData(leftRight, spiData) {
        if (leftRight === 'left') {
            ...
            {
                // Center and max are mixed up here.
                this.userCalibration.rightStick.x.max = ((spiData[1+2] << 8) & 0xF00) | spiData[0+2];
                this.userCalibration.rightStick.y.max = (spiData[2+2] << 4) | (spiData[1+2] >> 4);

                this.userCalibration.rightStick.x.center = ((spiData[4+2] << 8) & 0xF00) | spiData[3+2];
                this.userCalibration.rightStick.y.center = (spiData[5+2] << 4) | (spiData[4+2] >> 4);
